...
function FormStore({title,sxButton,iconButton,endpoint,nama,alamat,table}) {
    ...
    const handleSubmit = async () => {
        try{
            await axios.post(endpoint,{name : data,alamat : dataAlamat})
            showAlert("Berhasil!",`Store berhasil di${title.toLowerCase()}`, "success")
  ...
  return (
    <LayoutModal ref={modalTambah} height='auto' closeButton={false} width={ isMobile ? "80%" : "30%"} sxButton={sxButton} iconButton={iconButton}>
        <h2 className='text-1xl font-bold mb-4 text-center'>{title} STORE</h2>
        <Input defaultValue={nama} onChange={e => setData(e.target.value)} fullWidth label="Nama"/>
        <div className='mt-4'>
            <Input defaultValue={alamat} onChange={e => setDataAlamat(e.target.value)} fullWidth label="Alamat"/>
        </div>
        <div className='mt-3'>
