...
function FormEdc({title,sxButton,iconButton,endpoint,nama,table, description}) {
    ...
    const handleSubmit = async () => {
        try{
            await axios.post(endpoint,{name : data, description : dataDescription})
            showAlert("Berhasil!",`EDC berhasil di${title.toLowerCase()}`, "success")
  ...
  return (
    <LayoutModal ref={modalTambah} height='auto' closeButton={false} width={ isMobile ? "80%" : "30%"} sxButton={sxButton} iconButton={iconButton}>
        ...
        <div className='mt-4'>
            <Input defaultValue={description} onChange={e => setDataDescription(e.target.value)} fullWidth label="Description"/>
        </div>
        <div className='mt-3'>
