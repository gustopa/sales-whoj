...
import FormJenis from './components/FormJenis'
import Swal from 'sweetalert2'
import { showAlert } from '../../helper'
function Jenis({access}) {
    ...
    const handleDelete = (row_id) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            try{
              await axios.delete(`/item_type/delete/${row_id}`)
              showAlert("Berhasil!","Jenis berhasil dihapus",'success')
    ...
    const [columnDefs] = useState([
        {field : "row_id",headerName : "", minWidth : 110, width : 110,pinned : 'left', resizable : false, filter : false,hide : access == 'Read only' ? true : false,
            headerComponent : params => (
                <FormJenis table={table} endpoint="/item_type/tambah" title="TAMBAH" sxButton={{background:"#2e7d32",py:4}} iconButton={<FaCirclePlus className='text-white'/>} />
            ),
            cellRenderer : params => 
                (
                    <div key={params.value}>
                        <FormJenis table={table} nama={params.data?.name} endpoint={`/item_type/edit/${params.value}`} title="EDIT" sxButton={{minWidth : "30px", background : "#1976d2",padding : 3}} iconButton={<MdEdit color='white'/>} />
  ...
  return (
    <Layout title="Jenis" page="Jenis">
        <Table ref={table} columnDefs={columnDefs} endpoint="/item_type/getAll"/>
    </Layout>
  )
}
