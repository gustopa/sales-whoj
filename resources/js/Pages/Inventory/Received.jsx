...
function Received({access}) {
    const [columnDefs] = useState([
        {field : "row_id",headerName : "", pinned : "left",filter : false, width : 150, minWidth : 150,resizable : false,
            ...
            cellRenderer : params => 
                (
                    <div key={params.value}>
                        {params.data?.status != "RECEIVED" && params.data?.status != "CANCELLED" && access == "Full control" &&
                            <Link>
  ...
  return (
    <Layout title="Penerimaan inventory" page="Penerimaan inventory">
        <Table columnDefs={columnDefs} endpoint="/inventory_out/getAll?status[]=SEND&status[]=RECEIVED"/>
    </Layout>
  )
}
